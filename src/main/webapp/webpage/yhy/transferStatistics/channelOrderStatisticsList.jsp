...
+		var orderCompletedTime_begin = $("input[name='orderCompletedTime_begin']")
		...
		$.ajax({
					url : "transferStatisticsController.do?getChannelTotal&orderCompletedTime_begin="
	...
	$(function() {
		$(function(){
			...
			for(var i=0;i<obj.data.length;i++){
	 		...
	 		var a4 = '</select></span>';
	 		$("#channelOrderStatListForm").append(a1+a2+a3+c1+a4);
	 	});
		gettotal();
	});
	...
	function OrderStatListsearch() {
		try {
			if (!$("#channelOrderStatListForm").Validform({
				tiptype : 3
			}).check()) {
				return false;
			}
		...
		if (true) {
			var queryParams = $('#channelOrderStatList').datagrid('options').queryParams;
			$('#channelOrderStatListtb').find('*').each(function() {
				queryParams[$(this).attr('name')] = $(this).val();
			});
			$('#channelOrderStatList')
					.datagrid(
							{
