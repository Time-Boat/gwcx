 ...
 * @date 2017年11月23日 下午2:05:55
 */
public class SendMailUtil {
	// public static void sendMail(String toUser) throws Exception{
	// //1.创建链接，链接到邮箱服务器
	// final Properties properties = new Properties();
	// properties.load(SendMailUtil.class.getResourceAsStream("/config.properties"));
	...
	// message.setFrom(new
	// InternetAddress(properties.getProperty("from.user")));//设置发件人
	// message.setRecipient(RecipientType.TO, new
	// InternetAddress(properties.getProperty("to.user")));
	// message.setSubject("生日祝福");//设置邮件的主题
	// message.setContent("<h1>生日祝福</h1><p>祝你生日快乐！</p>",
	...
	private static final String COMPANY = "小龙出行";
	...
	static{
		...
		try {
			properties.load(SendMailUtil.class.getResourceAsStream("/mailConfig.properties"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void sendMail(String subject, String content, String[] toUsers) throws Exception {
		...
		transport.connect(properties.getProperty("from.user"), properties.getProperty("from.password"));
	 ...
	 * @throws Exception
	 */
	public static MimeMessage createMimeMessage(Session session, String sendMail,String subject, String content, String[] receiveMails) throws Exception {
		// 1. 创建一封邮件
		MimeMessage message = new MimeMessage(session);

		// 2. From: 发件人（昵称有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改昵称）
		message.setFrom(new InternetAddress(sendMail, COMPANY, "UTF-8"));

		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMails[0]));
