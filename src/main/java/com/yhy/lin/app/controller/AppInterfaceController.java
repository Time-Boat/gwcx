...
public class AppInterfaceController extends AppBaseController {
		...
		try {
			...
			try {
				...
				if (StringUtil.isNotEmpty(token)) {
					CarCustomerEntity user = null;
					synchronized (this) {
						List<CarCustomerEntity> userList = systemService.findByQueryString(
								"from CarCustomerEntity where token='" + token + "' and userType='" + userType + "'");
						...
						if (userList.size() > 0) {
							...
							String dateString = formatter.format(AppUtil.getDate());
							StringBuffer updateSql = new StringBuffer();
							updateSql.append(" UPDATE car_customer set login_count = ");
							updateSql.append(user.getLoginCount() + 1);
							updateSql.append(",last_login_time= '");
							updateSql.append(dateString);
	...
	public void getSmscode(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DealerInfoEntity> dealer = systemService
					.findHql("from DealerInfoEntity where status = 0 and phone = ? ", mobile);
			...
			if (!mobile.matches(AppGlobals.CHECK_PHONE)) {
				...
				case "3":
					...
					if (da != null) {
						...
						long l = systemService.getCountForJdbcParam(
								"select count(*) from dealer_info where phone=? and status='0' ",
								new Object[] { mobile });
						if (l > 0) {
	...
	public void createOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 验证参数
			JSONObject jsondata = checkParam(param, "orderTrainnumber", "orderFlightnumber", "refundPrice", "stationX",
					"stationY", "isException");
			...
			if (m < maxMinute) {
				...
				if ("0".equals(userType)) {
					...
					List<DealerInfoEntity> dealer = systemService
							.findHql("from DealerInfoEntity where status = 0 and phone = ? ", userPhone);
					String tPrice = appService.getCarTypePrice(t.getOrderNumbers(), t.getLineId(), userPhone,
							dealer.get(0).getDealerDiscount());
				...
				if (StringUtil.isNotEmpty(t.getId())) {
					...
					// 订单状态
					// 0：订单已完成。1：已付款待审核。2：审核通过待发车3：取消订单待退款。4：取消订单完成退款。5：拒绝退款。6：未支付
					// 默认进来是未支付的
					t.setOrderStatus(6);
	...
	public void getStationList(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 验证参数
			checkParam(new String[] { "serveType", "stationId", "cityId", "userType" }, serveType, stationId, cityId,
					userType);
	...
	public void getOrderStation(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> lm = systemService.findForJdbc(
					" select b.name,b.id,lb.siteOrder from transferorder t join lineinfo l on t.line_id = l.id join line_busstop lb on l.id = lb.lineId "
							+ " join busstopinfo b on b.id = lb.busStopsId " + " where t.id = ? order by lb.siteOrder",
					orderId);
	...
	public void personalInfo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> map = systemService.findForJdbc(
					"select b.name,b.stopLocation from customer_common_addr c inner join busstopinfo b on c.station_id=b.id where user_id = ? ",
					userId);
	...
	public void uploadPersonalInfo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			logger.info("uploadPersonalInfo   前端传递参数：  imgName=" + pName + "    userId=" + userId + "    address="
					+ address + "    userName=" + userName + "    idCard=" + idCard);

			CarCustomerEntity cc = systemService.getEntity(CarCustomerEntity.class, userId);
			String path = AppGlobals.IMAGE_BASE_FILE_PATH;
			if (StringUtil.isNotEmpty(imagesBaseBM)) {
				// 获取图片存储路径
				imgUrl = AppGlobals.APP_USER_FILE_PATH + userId // + "_" +
