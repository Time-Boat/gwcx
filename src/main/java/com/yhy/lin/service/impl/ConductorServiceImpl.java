...
public class ConductorServiceImpl extends CommonServiceImpl implements ConductorServiceI {
	...
	public JSONObject getDatagrid(DataGrid dataGrid, ConductorEntity conductors, String cr_bg, String cr_en,
		...
		if (StringUtil.isNotEmpty(lineId)) {
			queryCondition.append(" and cl.line_id = '" + lineId + "' ");
		}
		...
		String sqlCnt = " select count(*) from conductor d LEFT JOIN t_s_depart t on t.ID= d.departId LEFT JOIN t_s_base_user u on u.ID=d.create_user_id "
		...
		sql.append(
				" select d.id,d.name,d.phoneNumber,d.age,d.sex,d.create_date,d.status,d.delete_flag,d.conduct_status,d.application_status "
				+ " ,d.apply_content,u.username from conductor d LEFT JOIN t_s_depart t on t.ID= d.departId LEFT JOIN t_s_base_user u on u.ID=d.create_user_id "
				+ queryCondition.toString());
