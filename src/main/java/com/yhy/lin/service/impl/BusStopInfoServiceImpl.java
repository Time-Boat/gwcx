...
public class BusStopInfoServiceImpl extends CommonServiceImpl implements BusStopInfoServiceI{
	...
	public JSONObject getDatagrid3a(BusStopInfoEntity busStopInfo,DataGrid dataGrid,LineInfoEntity lineInfo){
		...
		if("3".equals(lineInfo.getType()) || "5".equals(lineInfo.getType())){
			sql.append("and a.id not in(select se.startId from start_end se where se.station_status='1' and se.endId='"+lineInfo.getEndLocation()+"' and se.lineType like '%"+lineInfo.getType()+"%')");
			sql.append("and a.id not in(select se.startId from start_end se where se.line_id='"+lineInfo.getId()+"' and se.lineType like '%"+lineInfo.getType()+"%')");
		}
	...
	public JSONObject getDatagrid4(String lineInfoId, DataGrid dataGrid){
			...
			// 取出总数据条数（为了分页处理, 如果不用分页，取iCount值的这个处理可以不要）
			String sqlCnt = "select count(*) from busstopinfo a inner join line_busstop_history b on a.id=b.busStopsId  inner join  lineinfo_history c on b.lineId = c.id ";
			if (!sqlWhere.isEmpty()) {
				sqlCnt += " where" + sqlWhere;
			}
			Long iCount = getCountForJdbcParam(sqlCnt, null);
			...
			// 取出当前页的数据 
			String sql = "select a.id as id,b.id as line_busstopId,a.name,b.siteOrder,a.status,b.arrivalTime from busstopinfo a inner join line_busstop_history b on a.id=b.busStopsId  inner join  lineinfo_history c on b.lineId = c.id ";
			if (!sqlWhere.isEmpty()) {
				sql += " where" + sqlWhere;
			}
			sql +=" order by b.siteOrder is NULL,b.siteOrder ";
	...
	//根据线路lineInfoId，拿到对应线路的未挂接的站点信息   import com.yhy.lin.entity.LineInfoEntity;
		...
		public JSONObject getDatagrid3b(BusStopInfoEntity busStopInfo,DataGrid dataGrid,LineinfoHistoryEntity lineInfo){
			String sqlWhere = getSqlWhere4(busStopInfo,lineInfo);
			...
			//如果是班车线路
			if("0".equals(lineInfo.getType())){
				sqlCnt.append(" and station_type = '0' and a.id not in(select busstopsid from line_busstop_history where lineId='"+lineInfo.getId()+"') ");
			}
			...
			//如果是班车线路
			if("0".equals(lineInfo.getType())){
				sql.append(" and station_type = '0' and a.id not in(select busstopsid from line_busstop_history where lineId='"+lineInfo.getId()+"') ");
			}
		...
		public String getSqlWhere4(BusStopInfoEntity busStopInfo,LineinfoHistoryEntity lineInfo){
			StringBuffer sqlWhere = new StringBuffer(" and a.deleteFlag='0' and a.station_type = '0' "); 
			if(StringUtil.isNotEmpty(lineInfo.getCityId())){
				sqlWhere.append(" and  a.cityId = '"+lineInfo.getCityId()+"' ");
			}
