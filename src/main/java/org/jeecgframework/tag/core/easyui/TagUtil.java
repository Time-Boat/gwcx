...
//			jsonTemp.append("\"name\":\"合计\",");
	...
	public static String getAutoList(Autocomplete autocomplete, List list) throws Exception {
		...
		jsonTemp.append("]}");
		return JSON.toJSONString(jsonTemp).toString();
	}
	/**
	...
	private static JSONObject getJson(DataGrid dg) {
		...
		try {
			if(!StringUtil.isEmpty(dg.getFooter())){
				jObject = JSON.parseObject(listtojson(dg.getField().split(","), dg.getTotal(), dg.getResults(),dg.getFooter().split(",")));
			}else{
				jObject = JSON.parseObject(listtojson(dg.getField().split(","), dg.getTotal(), dg.getResults(),null));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static JSONObject getJson(DataTableReturn dataTable,String field) {
		JSONObject jObject = null;
		try {
			jObject = JSON.parseObject(datatable(field, dataTable.getiTotalDisplayRecords(), dataTable.getAaData()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return jObject;
